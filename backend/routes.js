...
router.post( '/exerciseList', function ( req, res ) {
    pool.getConnection( function ( err, conn ) {
        if ( err ) { res.status( 400 ).json( "Could not connect to database, check server" ); }
        else {
            pool.query( 'SELECT * FROM exercises', function ( err, useWorkouts, fields ) {
...
//returns all information about all the workuts and sends back in useable JSON
router.get( '/allWorkouts', function ( req, res ) {
    pool.getConnection( function ( err, conn ) {
        if ( err ) { res.status( 400 ).json( "Could not connect to database, check server" ); }
        else {
            pool.query( 'SELECT * FROM workoutExercises', function ( err, tempData, fields ) {
                if ( err ) { { res.status( 404 ).json( err.message ); } }
                else {
